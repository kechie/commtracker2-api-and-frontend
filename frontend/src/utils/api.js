...
// Tracker API calls
export const getTrackers = async (page = 1, limit = 10, sortBy = 'dateReceived', sortOrder = 'DESC', search = '', hasRecipients = '', replyRequired = '') => {
  try {
...
export const getTrackerRecipients = async (trackerId) => {
  try {
    const response = await api.get(`/trackers/${trackerId}/recipients`);
    return response.data;
  } catch (error) {
    console.error('API Get Tracker Recipients Error:', error.response?.data || error.message);
    throw error.response?.data || error;
  }
};
...
export const getAttachment = async (recipientId, trackerId) => {
  try {
...
export const getTrackerAttachment = async (trackerId) => {
  try {
...
export const getPublicRoutingSlip = async (serialNumber) => {
  try {
    const response = await api.get(`/public/${encodeURIComponent(serialNumber)}/routing-slip`);
    return response.data;
  } catch (error) {
